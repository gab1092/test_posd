...
fecha_base=datetime(1994,9,30,18) #Hora UT
...
for nombre in nombres:
    ...
    release=gs.PointLineRelease(num_elements=5000,release_time=start_time,start_position=(lon,lat,0),end_release_time=start_time+gs.days(duracion_derrame))
    ...
    salida1='/LUSTRE/CIGOM/DERRAMES_2024_pygnome/salidas_90dias/' + nombre
    ...
    if cond==False:
        os.mkdir('/LUSTRE/CIGOM/DERRAMES_2024_pygnome/salidas_90dias/' + nombre)
       # print('Nuevo directorio creado')
        
           
    shape_file = os.path.join(salida1 + '/'+ nombre + '_' + str(year) + '_' +str(mes)+ '_' + str(dia1))
                          
    model.outputters += gs.ShapeOutput(shape_file,zip_output=True,output_timestep=gs.hours(12))
