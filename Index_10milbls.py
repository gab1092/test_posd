...
for name in lista:
                 ...
                 for cont in columnas: 
                     ...
                     datos2.loc[(datos2["Indexes"]>=0) & (datos2[columna]>=0)  & (datos2["Porc_min"]==0),["Porc_min"]]=datos2[columna].loc[(datos2["Indexes"]>=0) & (datos2[columna]>=0)  & (datos2["Porc_min"]==0)].astype(float)
                 ...
                 datos['Ind']=(datos['Porc_Min']+datos['Porc_Max']+(datos['Texp']/90)+Con_norm)/4
                # datos['Ind2']=datos['Porc_Max']*datos['Porc_Max']*(datos['Texp']/90)*Con_norm
                 ...
                 nombre=nombre[9:]
                 nombre2="/home/gaby.resendiz/Indice_10milbls/Indice"+nombre
...
#datos=read_dataframe(name)
#datos2=datos.drop(columns=['Id','GRID_ID','X','Y','GRID_ID_1','geometry','Area_km'])
#datos2["Indexes"]= datos2["xtupla"].str.find('cFish')
...
#        datos2=datos2.rename(columns={columna:int(cont)})
 ...
 #                datos2=datos2.drop(columns=['ID_1','xtupla','region','Región','layer','path','nueti','Contador'])
...
#                 datos2=datos2.drop(columns=['Dia','Indexes','Porc_min'])
                 ...
                 #datos['Porc_Min']=datos2.min(axis=1)
                # datos['Dia_min']=datos2.idxmin(axis=1)
  ...
  #               nombre=os.path.basename(name)
  #               nombre=nombre[9:]
  #               nombre2="/home/gaby.resendiz/Conc_prob_promedio30mil/prueba_"+nombre
  #               datos.to_file(filename=nombre2,driver='ESRI Shapefile')
  #               datos=[]
...
#atos3['Con_max']=datos3.max(axis=1)

#datos['Conc_max']=datos3['Conc_max']

#Con_norm=(datos['Conc_max']/np.max(datos['Conc_max']))

##sumado considerando un sistema de puntos del 0-4
#muy bajo,bajo,medio,alto,muy alto

#datos['Ind']=(datos['Porc_Min']+datos['Porc_Max']+(datos['Texp']/90)+Con_norm)/4
#datos['Ind']=(datos['Porc_Min']+datos['Porc_Max']+(datos['Texp']/90)+Con_norm)/4
#datos['Ind2']=datos['Porc_Max']*(datos['Texp']/90)*Con_norm

#/np.max(datos['Porc_Min']+datos['Porc_Max']+(datos['Texp']/90))
